...
void sampleRandomFunctionWithNoise(const std::string& fileName_vtk, const std::string& outputFolder) {
    ...
    std::cout << "sampling random function with noise..." << std::endl;
    std::cout << "NX = " << NX << " NY = " << NY << " NZ = " << NZ << " NR = " << NR << " NK = " << NK << std::endl;

    // fft plans
    fftw_plan plan_R_isf_r2c, plan_R_isf_c2r;
    ...
    plan_R_isf_c2r = fftw_plan_dft_c2r_2d(NY, NX, reinterpret_cast<fftw_complex*>(frHat), fr, FFTW_ESTIMATE);
    ...
    //debugging
    std::ofstream debugOut(outputFolder+"sampled_random_function.txt", std::ios::out); //overwrite
    for (int i=0; i<NR; ++i) {
        debugOut << fr[i] << std::endl;
    }
}
...
void sampleCorrelationWithNoise(const std::string& fileName_vtk, const std::string& outputFolder) {
    ...
    std::cout << "sampling the spatial correlation with noise..." << std::endl;
    std::cout << "NX = " << NX << " NY = " << NY << " NZ = " << NZ << " NR = " << NR << " NK = " << NK << std::endl;

    // fft plans
    fftw_plan plan_R_isf_r2c, plan_Rcor_c2r;
...
int main() {
    ...
    // sample the random function with noise
    sampleRandomFunctionWithNoise(fileName_vtk, outputFolder);

    // sample the spatial correlation with noise
